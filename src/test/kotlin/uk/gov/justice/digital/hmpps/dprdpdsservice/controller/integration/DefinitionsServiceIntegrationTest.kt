...
class DefinitionsServiceIntegrationTest {
  ...
  fun `Definitions API returns the full list of definitions for a valid directory which has definition json files`() {
    ...
    val expectedJson2 = this::class.java.classLoader.getResource("dev/definitions/$requestPath/ors-transfer-holds-report.json")?.readText()
    val expectedJson3 = this::class.java.classLoader.getResource("dev/definitions/$requestPath/ors-users-in-a-specific-role.json")?.readText()
    val expectedJson4 = this::class.java.classLoader.getResource("dev/definitions/$requestPath/external-movements-with-summaries.json")?.readText()
